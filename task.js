...
function purchaseBook(books, purchasedAmount, creditTermMonths) {
   ...
   for (let i = 1; i <= creditTermMonths; i++) {
      ...
      let pricePerMonth = totalPrice / creditTermMonths;
      dueDates.push({ dueDate: dueDate.toDateString(), paymentAmount: pricePerMonth });
   }

   let paymentCheck = 0;
   dueDates.forEach((element) => (paymentCheck += element.paymentAmount));
   ...
   if (creditTermMonths > 1) {
      if (purchasedAmount > stockAvailable) {
         ...
         // dueDates.forEach((element) => console.log("Date:", element.dueDate, ", Payment:", element.paymentAmount));
         console.log("Check if total payment is same:", totalPrice, "=", paymentCheck);
      } else {
         console.log("Purchased Amount:", purchasedAmount);
         console.log("Total Price:", totalPrice);
         console.log("Total Credits:", creditTermMonths);
